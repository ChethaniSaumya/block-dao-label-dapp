...
for m in missing:
    ...
    escaped_kr = escaped_kr.replace('\uFFFD', '·')
    
    lines.append(f'  "{escaped_m}": "{escaped_kr}",')

# Notice we use "\n" to join them with REAL newline characters now!
block = "\n".join(lines) + "\n"
